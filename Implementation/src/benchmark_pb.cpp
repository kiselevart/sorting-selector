...
PYBIND11_MODULE(benchmarks_pb, m) {
    ...
    m.def("benchmark_sorter", [](const std::string& sorter_name, const std::vector<std::vector<int>>& arrays) {
        ...
        else if (sorter_name == "drop_merge_sort") {
            sort_func = create_sorter_func<cppsort::drop_merge_sorter>();
        }
        ...
        else if (sorter_name == "split_sort") {
            sort_func = create_sorter_func<cppsort::split_sorter>();
        }
        ...
        else if (sorter_name == "verge_sort") {
            sort_func = create_sorter_func<cppsort::verge_sorter>();
        }
        else {
